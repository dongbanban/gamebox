...
**What to build:** 在不改变现有关卡可观察结果的前提下，拆分关卡生成器职责并建立统一关卡提供器，使后续不规则棋盘、部分重叠与可解搜索改造可以在明确模块边界内完成。
...
- 首关 canonical 配置继续由 provider 返回固定关卡；显式首关 seed/version 继续走生成器并可 replay，避免吞掉请求身份。

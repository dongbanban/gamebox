...
- 实现：十种图案 SVG 按图片 2 行优先顺序写入 `public/assets/dog-icons/`，映射维护在 `game-assets.ts`；棋盘、暂存槽、logo 复用同一资产 fn；目录封面改为第 10 项傻狗；统计保持同排，缩略图列宽提升。

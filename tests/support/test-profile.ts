...
export type DogV13TestProfile =
  (typeof DOG_TEST_PROFILES.profiles)[DogV13TestProfileName];
...
export function resolveDogTestProfileName(
...
export function getDogTestProfile(profileName?: string): DogV13TestProfile {
  ...
  if (profile === undefined)
    throw new Error(`狗了个狗 test profile has no data: ${resolvedName}`);
  return profile;
}
...
export function formatDogGenerationTestReport(
  ...
  return [
    ...
    ...(testCase.levelSeed === undefined
      ? []
      : [`levelSeed=${testCase.levelSeed}`]),

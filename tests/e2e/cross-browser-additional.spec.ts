...
test("跨浏览器减少动态效果并保留静态识别信息", async ({ page }) => {
  ...
  const illusion = page
    .locator('[data-testid="dog-block"][data-special-mechanism="illusion"]')
    .first();
  ...
  const illusionVisual = await illusion.evaluate((element) => {
    ...
    return {
      ...
      fuzzyAnimation:
        glyph === null ? null : getComputedStyle(glyph).animationName,
    };
  });
  ...
  const twin = page
    ...
    .first();
  const magnetic = page
    .locator('[data-testid="dog-block"][data-special-mechanism="magnetic"]')
    .first();
  ...
  const traySlot = page
    .locator('[data-testid="dog-tray-slot"][data-pattern-type]')
    .first();
  await expect(traySlot).toHaveCount(1);
  expect(
    await traySlot.evaluate(
  ...
  const thumbnailVisual = await thumbnail.evaluate((element) => ({
    ...
    borderWidth: getComputedStyle(element).borderTopWidth,
    maskAnimation: getComputedStyle(element, "::before").animationName,
    fuzzyAnimation: getComputedStyle(
      element.querySelector(".dog-block__glyph--fuzzy") ?? element,
    ).animationName,
  }));
...
test("跨浏览器新获得的焦点不会被旧棋盘焦点抢回", async ({ page }) => {
  ...
  const targetId = await page.evaluate((currentFocusedId) => {
    return (
      [...document.querySelectorAll('[data-testid="dog-block"]')]
        .find(
          (block) =>
  ...
  const lockedDuringSelection = await page.evaluate((blockId) => {
    ...
    document
      ...
      ?.focus();
    return (
      document.querySelector<HTMLElement>('[data-testid="dog-game"]')?.dataset
        .inputLocked === "true"
    );
  }, targetId);
...
test("跨浏览器键盘可以遍历并激活方块", async ({ page, browserName }) => {
    ...
    for (
      ...
      activatedId = await page.evaluate(() => {
        ...
        return active instanceof HTMLElement &&
          active.dataset.testid === "dog-block"
  ...
  await page
    .locator(`[data-testid="dog-block"][data-block-id="${activatedId}"]`)
    .press("Enter");
  await expect(
    page.locator(`[data-testid="dog-block"][data-block-id="${activatedId}"]`),
  ).toHaveCount(0);
  expect(
    await page
      .locator('[data-testid="dog-tray-slot"][data-pattern-type]')

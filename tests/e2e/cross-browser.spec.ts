...
test("跨浏览器核心 smoke：注册、目录与首关入口可用", async ({ page }) => {
  ...
  const modalPosition = await page
    .getByTestId("dog-loadout-modal")
    .evaluate((modal) => getComputedStyle(modal).position);
...
test("龇牙狗图案在跨浏览器中保留白色牙齿", async ({ page }) => {
  ...
  const catalogCoverSource = await page
    .locator(".catalog-item__cover")
    .getAttribute("src");
...
test("跨浏览器三消期间道具栏保持稳定", async ({ page }) => {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const before = await page.evaluate(() => {
      ...
      const block = [
        ...
        .sort((first, second) => {
          ...
          return (
            (trayCounts.get(secondPattern) ?? 0) -
              (trayCounts.get(firstPattern) ?? 0) ||
    ...
    await page
      .locator(`[data-testid="dog-block"][data-block-id="${before.blockId}"]`)
      .click();
    ...
    if (after?.match === true) {
      ...
      expect(
        await page
          ...
          .getAttribute("data-cross-browser-probe"),
      ).toBe("true");
      break;
    }
  }
...
test("跨浏览器乱序与复原反馈期间锁定重玩并保持窄屏布局", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const state = JSON.parse(
      window.localStorage.getItem("gamebox.state") ?? "{}",
    );
  ...
  for (
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const blockId = await page.evaluate(() => {
        ...
        const trayPatterns = new Set(
          trayBlocks.map((slot) => slot.dataset.patternType),
        );
        ...
        return (
          [
            ...
            .sort(
              (first, second) =>
                Number(second.dataset.z) - Number(first.dataset.z),
            )
    ...
    expect(shuffleLayout.right).toBeLessThanOrEqual(
      shuffleLayout.viewportWidth,
    );
    ...
    restoreReady = await page
      .locator('[data-item-id="restore-whistle"]')
      .isEnabled();
    if (!restoreReady) {
      ...
      expect(loadingLayout.right).toBeLessThanOrEqual(
        loadingLayout.viewportWidth,
      );
      expect(loadingLayout.bottom).toBeLessThanOrEqual(
        loadingLayout.viewportHeight,
      );
      await expect(page.getByTestId("dog-game")).toBeVisible({
        timeout: 120_000,
      });
      const nextRunSeed = await page
        .getByTestId("dog-game")
        .getAttribute("data-run-seed");
  ...
  const replayedRunSeed = await page
    .getByTestId("dog-game")
    .getAttribute("data-run-seed");
  ...
  await page.waitForFunction(() => {
    ...
    return (
      lifecycle === undefined || lifecycle.created === lifecycle.terminated
    );
  });
});
...
test("跨浏览器加载态保持窄屏布局", async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    workerPrototype.postMessage = function (
      this: Worker,
      message: unknown,
...
test("跨浏览器失败结果态保持窄屏布局", async ({ page }) => {
  ...
  const resultLayout = await page
    .locator('[data-view="game-result"]')
    .evaluate((view) => {

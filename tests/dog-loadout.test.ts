...
describe("狗了个狗道具组", () => {
  it("固定提供八种可展示道具，并要求选择三个不同 ID", () => {
    ...
    expect(
      DOG_ITEM_DEFINITIONS.every((item) => item.name && item.description),
    ).toBe(true);
    ...
    expect(
      isValidDogLoadout(["triple-removal", "tray-capacity", "wildcard"]),
    ).toBe(true);
    expect(isValidDogLoadout(["triple-removal", "tray-capacity"])).toBe(false);
    expect(
      isValidDogLoadout(["triple-removal", "triple-removal", "wildcard"]),
    ).toBe(false);
    expect(
      isValidDogLoadout(["triple-removal", "tray-capacity", "unknown"]),
    ).toBe(false);
  });
  ...
  it("以浮层渲染选组，并把已选道具渲染成摘要缩略图", () => {
    ...
    expect(editor).toMatch(
      /data-action="confirm-loadout"[^>]*>\s*确认\s*<\/button>/,
    );
    ...
    expect(summary.match(/data-testid="dog-loadout-thumbnail"/g)).toHaveLength(
      3,
    );
    ...
    expect(summary).toContain(
      'src="assets/dog-item-icons/tray-capacity-plus-one.svg"',
    );
    ...
    expect(
      summary.match(/data-testid="dog-loadout-thumbnail-uses"/g),
    ).toHaveLength(3);

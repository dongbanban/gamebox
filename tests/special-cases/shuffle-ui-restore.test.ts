...
describe("特殊机制测试 · shuffle-ui · 复原", () => {
  it("复原哨在乱序动画后开放，反向反馈期间锁定输入并在结束后复原", async () => {
    ...
    const shuffledIds = game
      .getState()
      .session.trayBlocks.map((block) => block.id);
    expect(
      game
        .getState()
        .items?.items.find((item) => item.id === "restore-whistle"),
    ).toMatchObject({ available: true, remainingUses: 1 });
    const whistle = root.querySelector<HTMLButtonElement>(
    ...
    expect(
      root.querySelector<HTMLElement>('[data-testid="dog-shuffle-effect"]')
        ?.dataset.shuffleOutcome,
    ).toBe("restored");
    ...
    expect(game.getState().session.trayBlocks.map((block) => block.id)).toEqual(
      ["shuffle", "single-1", "licking-1", "guard-1", "mad-1"],
    );
    expect(
      game.getState().session.trayBlocks[0]?.specialMechanism,
    ).toBeUndefined();
    expect(
      game
        .getState()
    ...
    try {
      ...
      const beforeShuffleSlots = new Map(
        [...tray.querySelectorAll<HTMLElement>("[data-block-id]")].map(
          (slot) => [slot.dataset.blockId, slot] as const,
        ),
      );
        ...
        reorderMutations
          ...
          .every(
            (node) =>
              ...
              beforeShuffleChildren.includes(node),
          ),
      ).toBe(true);
      ...
      root
        .querySelector<HTMLButtonElement>('[data-item-id="restore-whistle"]')
        ?.click();
      await vi.runAllTimersAsync();
      const restoreMutations = restoreObserver.takeRecords();
      restoreObserver.disconnect();

      const restoredIds = game
        .getState()
        .session.trayBlocks.map((block) => block.id);
      expect(restoredIds).toEqual([...beforeShuffleIds, "mad-1"]);
      expect(
        [...tray.querySelectorAll<HTMLElement>("[data-block-id]")].map(
          (slot) => slot.dataset.blockId,
        ),
      ...
      expect(
        restoreMutations
          ...
          .every(
            (node) =>
              node instanceof HTMLElement && shuffledChildren.includes(node),
          ),
      ).toBe(true);
  ...
  it("乱序结算使用最终快照并保留冻结进度与稳定节点", async () => {
    ...
    try {
      ...
      expect(
        game
          .getState()
          .session.trayBlocks.find((block) => block.id === "frozen"),
      ).toMatchObject({
        specialMechanism: {
          type: DOG_FREEZE_MECHANISM_TYPE,
      ...
      ).toEqual(game.getState().session.trayBlocks.map((block) => block.id));
      ...
      expect(
        tray.querySelector<HTMLElement>('[data-block-id="frozen"]')?.dataset
          .specialMechanismProgress,
      ).toBe("1");
      ...
      ).toEqual([...beforeSlots.keys(), "shuffle"]);
      expect(
        game
          .getState()
          .session.trayBlocks.find((block) => block.id === "frozen"),
      ...
      expect(
        game
          .getState()
          .session.trayBlocks.find((block) => block.id === "shuffle")
          ?.specialMechanism,
      ).toBeUndefined();
      expect(
        game.getState().items?.items.find((item) => item.id === "key"),
